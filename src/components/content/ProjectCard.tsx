...
export default function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white rounded-2xl overflow-hidden shadow-soft hover:shadow-glow-lg transition-all duration-300"
    >
      {/* Featured Badge */}
      {data.featured && (
      ...
      <div className="p-6">
        <div className="flex items-start justify-between mb-3">
          ...
          <time className="text-sm text-neutral-500 whitespace-nowrap ml-4">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {data.technologies.slice(0, 3).map((tech) => (
            <span
              key={tech}
              className="text-xs font-medium px-2 py-1 bg-neutral-100 text-neutral-700 rounded-full"
            >
              {tech}
            </span>
          ))}
